...
class MinhaJanela(QMainWindow):
    ...
    # Rótulo para a zona
        ...
        self.combo_escola_ano.setGeometry(310,linha,230,25)
    ...
    def inserir_registro(self):
        ...
        if not self.cpf_valido(vcpf):
            ...
            vsql="INSERT INTO tb_pessoa (T_NOME, T_RG, T_CPF, T_MAE, T_CPFMAE, T_PAI, T_CPFPAI, N_BAIRRO, T_NIS, T_FONE, T_CEP, T_GRAU_ENSINO, T_ESCOLA, T_SERIE_ANO) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', %s, '%s', '%s', '%s', '%s', '%s', '%s')" %(vnome, vrg, vcpf, vmae, vcpfmae, vpai, vcpfpai, vbairro[0], vnis, vfone, vcep, vgrau_ensino, vescola, vserieano)
            print(vsql)
            banco.atualizar(vsql)
            self.limpar_dados()
